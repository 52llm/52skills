...
总测试：**"如果有人说这是 AI 做的，别人会立刻信吗？"** 分三级：Critical=一眼 AI；Major=设计师会皱眉；Minor=好与卓越的差距。`scripts/ui_taste check` 能机械抓到的条目已标 ⚙。
...
- 暗底玻璃拟态+霓虹叠满全页。
- ⚙ div 拼的假产品截图/假终端/假仪表盘（#1 AI 印记）。用真截图、生成图、真组件迷你预览，或干脆不放。
...
- 时间序列用竖条图（应为线/面积图）；饼图 ≥5 片；带满底轨道的进度条当对比图。
