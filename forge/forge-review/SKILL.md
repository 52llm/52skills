...
阅读策略随规模缩放：**<20 个文件逐文件全读**再下判断；**20–100 先读 diff**，再深读高危文件（鉴权/支付/配置/迁移）；**>100 要求缩小评审范围**，不硬扫。
...
常驻镜头自带 **Fowler 异味基线**（12 个高信号坏味道+修法，清单见 lenses.md）。两条约束：仓库成文规范**永远压过**基线（规范背书的写法不报）；异味只作 judgement call（「疑似 Feature Envy」），永不作硬违规。
